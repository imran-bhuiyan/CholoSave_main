...
def analyze_individual(data):
    ...
    if investment_ratio < 20:
        tips.append({
            ...
            "tip": f"Your current investment ratio is {investment_ratio:.1f}%. Consider diversifying your savings by investing more through your groups. A balanced portfolio typically has 20-30% in investments.",
            "priority": "High"
        })
    elif investment_ratio > 60:
        tips.append({
            "category": "Risk Management",
            "tip": f"Your investment ratio is {investment_ratio:.1f}%. This is quite high, which could limit your liquidity in emergencies. Consider adjusting to maintain more liquid savings.",
            "priority": "Medium"
        })
    ...
    if total_loans > 0:
        completion_rate = (completed_loans / max(total_loans, 1) * 100)
    ...
    if len(contributions) > 0:
        ...
        if max_contribution > min_contribution * 3 and min_contribution > 0:
            tips.append({
                ...
                "tip": "Your group contributions are significantly uneven, with the highest contribution being more than three times the lowest. Consider balancing your savings across groups for better risk management.",
                "priority": "Medium"
            })
    ...
    if total_savings < 5000:
        tips.append({
            ...
            "tip": f"Your total savings are ${total_savings:.2f}. It's recommended to build an emergency fund of at least $5,000. Set up automatic weekly savings transfers to reach this goal.",
            "priority": "High"
        })
...
def analyze_group(data, group_id):
    ...
    goal_amount = max(float(group_data['goal_amount']), 1)
    emergency_fund = float(group_data['emergency_fund'])
    total_investments = float(group_data['total_investments'])
    active_members = max(int(group_data['active_members']), 1)
    days_active = max(int(group_data['days_active']), 1)
    ...
    if progress_percentage < 40:
        ...
        tips.append({
            ...
            "tip": f"Your group's savings progress is {progress_percentage:.1f}%. To reach your goal, each member should aim to save ${weekly_target:.2f} weekly. Consider increasing regular contributions.",
            "priority": "High"
        })
    ...
    if emergency_fund < recommended_emergency:
        tips.append({
            "category": "Emergency Fund",
            "tip": f"Your group's emergency fund is ${emergency_fund:.2f}, below the recommended 15% of total savings (${recommended_emergency:.2f}). Consider increasing it.",
            "priority": "High"
        })
    ...
    if investment_ratio < 20:
        tips.append({
            ...
            "tip": f"Your group's investment ratio is {investment_ratio:.1f}%. Consider increasing group investments to 20-30% of total savings for better long-term growth.",
            "priority": "Medium"
        })
    ...
    if active_members < 5:
        tips.append({
            ...
            "tip": f"Your group has only {active_members} members. Consider recruiting more members to increase the group's saving potential and spread risk.",
            "priority": "Medium"
        })
    
    if avg_daily_saving > 1000:
        tips.append({
            "category": "Growth Management",
            "tip": f"Your group's daily savings growth is ${avg_daily_saving:.2f}, which is strong. Consider diversifying into different investment types to maximize returns.",
            "priority": "Medium"
        })
...
def analyze_all_groups(groups):
    ...
    if investment_ratio < 15:
        tips.append({
            ...
            "tip": f"Your overall investment ratio is {investment_ratio:.1f}%. Consider increasing investments across groups to improve long-term returns.",
            "priority": "High"
        })
    ...
    if emergency_ratio < 10:
        tips.append({
            "category": "Risk Management",
            "tip": f"Total emergency funds across groups are ${total_emergency:.2f}, which is low. Consider building stronger safety nets.",
            "priority": "High"
        })
    ...
    if len(all_savings) > 1:
        ...
        if coefficient_of_variation > 50:
            tips.append({
                ...
                "tip": "There's significant variation in group performance. Consider standardizing saving practices across groups to reduce volatility.",
                "priority": "Medium"
            })
    ...
    if len(groups) > 0:
        ...
        if avg_members < 5:
            tips.append({
                ...
                "tip": f"The average number of members per group is {avg_members:.1f}. Consider recruiting more members to increase collective saving power.",
                "priority": "Medium"
            })
