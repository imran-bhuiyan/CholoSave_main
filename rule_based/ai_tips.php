...
try {
        ...
        SELECT 
            (COALESCE(SUM(s.amount), 0) - 
             COALESCE((SELECT SUM(w.amount) 
                      FROM withdrawal w 
                      WHERE w.user_id = ? 
                      AND w.status = 'approved'), 0)
            ) as total_savings,
            COUNT(DISTINCT lr.id) as total_loans,
            SUM(CASE WHEN lr.status = 'approved' THEN 1 ELSE 0 END) as active_loans,
    ...
    $stmt = $conn->prepare("
        SELECT 
            ...
            COUNT(DISTINCT CASE WHEN gm.status = 'approved' THEN gm.user_id END) as active_members,
            ...
            COUNT(DISTINCT CASE WHEN lr.status = 'approved' THEN lr.id END) as active_loans
        FROM group_membership gm
        JOIN my_group g ON gm.group_id = g.group_id
        LEFT JOIN savings s ON g.group_id = s.group_id
        LEFT JOIN investments i ON g.group_id = i.group_id
        LEFT JOIN loan_request lr ON g.group_id = lr.group_id
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div class="col-span-1 bg-gray-50 p-4 rounded-lg shadow">
                    ...
                    <div class="mb-4">
                        ...
                        <p class="text-sm text-gray-600">Active Loans: <?= $individual_data['active_loans'] ?></p>
                        <p class="text-sm text-gray-600">Completed Loans: <?= $individual_data['completed_loans'] ?></p>
                    </div>
                            ...
                            <?php foreach ($individual_contributions as $contribution): ?>
                                <li class="text-sm">
                                    ...
                                    <div class="text-xs text-gray-500">
                                        Goal Progress: <?= round(($contribution['contribution'] / max($contribution['goal_amount'], 1)) * 100, 1) ?>%
                                    </div>
                                </li>
                            <?php endforeach; ?>
                        </ul>
